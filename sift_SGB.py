...
if __name__ == '__main__':
    data_path = "../experience4/plant-seedlings-classification"
    ...
    print("开始建立词汇表===========================>")
    if not os.path.exists('../experience4/vocab.pkl'):
        vacob = build_vocabulary(train_image_paths, 100, 200)
        pickle.dump(vacob, open('../experience4/vocab.pkl', 'wb'))
    else:
        vacob = pickle.load(open('../experience4/vocab.pkl', 'rb'))
    print(vacob.shape)
    print("词汇表建立完成===========================>")
    # 打乱
    # train_image_paths, trian_image_labels_id= shuffle(train_image_paths, trian_image_labels_id, random_state=0)
    print("开始提取训练集BOW描述===========================>")
    if not os.path.exists('../experience4/train_X.pkl'):
        train_X = get_train_feat(train_image_paths, vacob, 100)
        pickle.dump(train_X, open('../experience4/train_X.pkl', 'wb'))
    else:
        train_X = pickle.load(open('../experience4/train_X.pkl', 'rb'))
    ...
    print("开始提取测试集BOW描述===========================>")
    if not os.path.exists('../experience4/test_X.pkl'):
        test_X = get_train_feat(test_image_paths, vacob, 100)
        pickle.dump(test_X, open('../experience4/test_X.pkl', 'wb'))
    else:
        test_X = pickle.load(open('../experience4/test_X.pkl', 'rb'))
    ...
    print(len(test))
    sample = pd.read_csv("../experience4/sample_submission.csv")
